...
%GENERATERANDOMGRID(obj) fills interval (Ia - Ib) with n-many grid points 
%   separated by at most k distance and at least l distance. If mesh size k
%   has been specified, GENERATERANDOMGRID ensures that one interval has
%   length k.
%
...
if isempty(obj.meshSize)
    obj.grid = generaterandomgrid(Ia,Ib,n,upperTolerance,lowerTolerance);
...
% if mesh size k is preset, ensure one interval has length k 
    ...
    k        = obj.meshSize;
    x        = zeros(1,n);
    x(1:n-1) = generaterandomgrid(Ia,Ib-k,n-1,upperTolerance,lowerTolerance);
    x(n)     = Ib;
